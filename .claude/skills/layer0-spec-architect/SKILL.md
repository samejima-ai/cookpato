...
4. モード判定（S/U/Rスコアリング + L2発動閾値チェック + LC記録）
...
ステップ1.5は LC ≥ 1 の場合のみ実行する。プロトコル詳細は `references/ritual-protocol.md` を参照。
...
- `references/regime-assessment.md` — モード判定プロトコル（S/U/Rスコアリング、L2発動閾値、LC判定）
