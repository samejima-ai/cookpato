...
- **長押しドラッグ並び替え**：`↑↓` ボタン廃止、`moveStockUp/Down` API 削除。`reorderStock(fromIndex, toIndex)` API 追加。500ms 長押し成立で `dragState` セット、pointermove で `toIndex` 更新、pointerup で `reorderStock` 確定。Escape／行外タップで解除。`setPointerCapture` を使い行をまたぐ移動を追跡。短時間タップ（長押し未成立）は編集モード進入として分岐。
...
#### 既知未検証事項
- 実機 iPhone 11 Safari での長押しドラッグ挙動（jsdom では `setPointerCapture` 経由のドラッグ追跡をフルにはシミュレートしていない）。Vercel Preview デプロイで実機確認推奨。
