...
```bash
git clone https://github.com/samejima-ai/cockpato.git
cd cockpato
```
...
2. **Vercel（または Cloudflare Pages）にデプロイ**
   - https://vercel.com/new で `samejima-ai/cockpato` を Import。
   - **Production Branch を `main` に設定**（デフォルトで main になるはず）。
   - 設定は自動検出でOK（Framework: Vite、Output: `dist`）。
   - 発行 URL（例: `https://cockpato-xxx.vercel.app`）を妻の iPhone 11 に送る。
