...
SPEC.md「完了トグル（品単位）」の調理中操作要件を実装する際の規約：

- **ヒット領域は44×44pxを上回り、行全体の左側1/3以上**を確保する
  - アイコン本体は小さくてよい。タップ可能領域を padding/疑似要素で広げる
- **視覚フィードバックは3点併用**：打ち消し線 ＋ 文字色グレーアウト ＋ 行背景色変化
  - 単一の視覚要素（チェックマークのみ）に依存しない
- **触覚フィードバック**を発火する：
  - iOS Haptic（利用可能な場合）／フォールバックは `navigator.vibrate(10)` 程度
  - 非対応端末でエラーにならないよう存在チェックを必ず入れる
- **チャタリング防止**：300ms以内の連続タップは1回として扱う
  - シングルタップの即時反映は損なわない
- **編集モード進入領域とトグル領域は物理的に分離**する
  - トグルタップで textarea にフォーカスが移らないこと
  - `e.stopPropagation()` 等で干渉を排除する

### フリー入力（常時textarea化）

SPEC.md「フリー入力 / 編集モードの扱い」を実装する際の規約：

- 可視範囲付近の日付行は textarea を常時マウントし、タップ不要で直接入力可能にする
- 範囲外の遠い日付行は DOM コスト抑制のためタップ進入方式でよい
- 範囲の境界判定は `Calendar` 側で行い、`DayRow` には props として渡す
- 入力は onChange で都度 `setMealsText` を呼ぶ（即時保存原則は不変）

### ストック削除UI

- 削除は**タップ操作のみ**。スワイプ・長押しは実装しない
  - 理由：iOS Safari PWA でページバックと誤動作する／調理中の片手操作と相性が悪い
- 各項目に明示的な削除ボタン（×等）を1つ配置し、44×44px以上の領域を確保する
